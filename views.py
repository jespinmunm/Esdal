...
def summary_recalculate(request):
    ...
    matrix_output=[[None for c in range(4)] for r in range(len(matrix_output))]
    i=-1
    ...
    with open(data) as outfile:
        ...
        try:
            request_url_upload='/api/assessment_result/v1'
...
def calculate():
    ...
    try:    
        ...
        for T_E in range (1, total_structures+1):
            ...
            cursor.execute('SELECT * FROM BBDD_SQL.dbo.halfjoint')
            ishalfjoint=0
            for row in cursor:
        ...
        matrix_output=[[None for c in range(4)] for r in range(total_structures)]
        i=-1
        for key in d_StructureKey:
            i=i+1
            matrix_output[i][0]=key
...
#        response.status=404
        if (response.status==200 and response.reason=='OK'):
            ...
            datatext=calculate() #datatext=write json name if ok   #datatext=1 if error
